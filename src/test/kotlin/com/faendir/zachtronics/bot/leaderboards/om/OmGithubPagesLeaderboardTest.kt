...
internal class OmGithubPagesLeaderboardTest {
    ...
    fun get() {
        ...
        expectThat(record!!.score).isEqualTo(OmScore(mapOf(HEIGHT to 1.0, CYCLES to 52.0, COST to 125.0).toMap(LinkedHashMap())))
    ...
    @Test
    fun update1() {
        val record = OmRecord(OmScore(mapOf(AREA to 200.0, CYCLES to 200.0, COST to 200.0).toMap(LinkedHashMap())), "http://fake.link")
