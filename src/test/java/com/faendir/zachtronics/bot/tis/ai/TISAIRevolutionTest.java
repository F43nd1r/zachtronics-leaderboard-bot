...
class TISAIRevolutionTest {
    ...
    static final List<TISPort> dstReplacements = List.of(NIL, LAST, LEFT, RIGHT, UP, DOWN, ANY).subList(0, 0);
            ...
            try (DirectoryStream<Path> paths = Files.newDirectoryStream(solutionsPath, puzzle.getId() + "*.txt")) {
                for (Path path : paths) {
                    ...
                    for (int delAmount = deleteMin; delAmount <= deleteMax; delAmount++) {
                        genDelete(save, puzzle, baseDescr, delAmount);
                    }
                    ...
                    for (int fuzzFactor = -fuzzSingle; fuzzFactor <= fuzzSingle; fuzzFactor++) {
                        genFuzzSingle(save, puzzle, baseDescr, fuzzFactor);
                    }
                    if (fuzzTries != 0) {
    ...
    private void genDelete(@NotNull TISSave save, TISPuzzle puzzle, String baseDescr, int delAmount) {
    ...
    private void genFuzzSingle(TISSave save, TISPuzzle puzzle, String baseDescr, int fuzzFactor) {
        if (fuzzFactor == 0)
            return;
        int foundLine = 0;
        boolean found = true;
        while (found) {
            ...
            if (found) {
                String descr = baseDescr + "?F" + fuzzFactor + "L" + foundLine;
                submit(data, puzzle, descr, null);
            }
        }
    }
