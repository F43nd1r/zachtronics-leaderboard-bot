...
class TISAIRevolutionTest {
    ...
    static final Set<TISOperation> jumpReplacerIgnore = EnumSet.of(HCF);
    ...
    static final int fuzzSingle = 0; // move immediates around by up to this value one by one
        ...
        for (TISPuzzle puzzle : repository.getTrackedPuzzles()) {
            ...
            try (DirectoryStream<Path> paths = Files.newDirectoryStream(solutionsPath, puzzle.getId() + "*.txt")) {
                for (Path path : paths) {
                    ...
                    if (noop) {
                        String descr = filename.replace(puzzle.getId() + ".", "");
                        submit(Files.readString(path), puzzle, descr, null);
                    }
                    for (int delAmount = deleteMin; delAmount <= deleteMax; delAmount++) {
                        genDelete(save, puzzle, filename, delAmount);
                    }
                    for (int swapSize = swapSizeMin; swapSize <= swapSizeMax; swapSize++) {
                        for (int swapDist = swapDistMin; swapDist <= swapDistMax; swapDist++) {
                            genSwap(save, puzzle, filename, swapSize, swapDist);
                        }
                    }
                    for (int dupeAmount = dupeMin; dupeAmount <= dupeMax; dupeAmount++) {
                        genDupe(save, puzzle, filename, dupeAmount);
                    }
                    for (TISOperation newJump : jumpReplacerNew) {
                        genJumpReplacer(save, puzzle, filename, newJump);
                    }
                    for (int labelShift = labelShiftMin; labelShift <= labelShiftMax; labelShift++) {
                        genLabelShift(save, puzzle, filename, labelShift);
                    }
                    if (fuseMovMovAcc) {
                        genFuseMovMovAcc(save, puzzle, filename);
                    }
                    if (movAddSwap) {
                        genMovAddSwap(save, puzzle, filename);
                    }
                    if (jmpExchange) {
                        genJmpExchange(save, puzzle, filename);
                    }
                    if (jmpExchange2) {
                        genJmpExchange2(save, puzzle, filename);
                    }
                    for (int fuzzFactor = -fuzzSingle; fuzzFactor <= fuzzSingle; fuzzFactor++) {
                        genFuzzSingle(save, puzzle, filename, fuzzFactor);
                    }
                    if (fuzzTries != 0) {
                        genFuzzTries(save, puzzle, filename);
                    }
                }
            }
    ...
    private void genDelete(@NotNull TISSave save, TISPuzzle puzzle, String filename, int delAmount) {
    ...
    private void genSwap(@NotNull TISSave save, TISPuzzle puzzle, String filename, int swapSize, int swapDist) {
        for (int codeLineIdx = 0; codeLineIdx + swapSize + swapDist < save.getInstructions(); codeLineIdx++) {
            ...
            String descr = filename.replace(puzzle.getId() + ".", "")
                                   .replace(".txt", "?s" + codeLineIdx + "-" + (codeLineIdx + swapSize) + "+" + swapDist);
            submit(data, puzzle, descr, null);
        }
    }

    private void genDupe(@NotNull TISSave save, TISPuzzle puzzle, String filename, int dupeAmount) {
        for (int dupeStart = 0; dupeStart + dupeAmount - 1 < save.getInstructions(); dupeStart++) {
            int codeCount = 0;
            StringBuilder data = new StringBuilder();
            ...
            for (TISNode node : save.getNodes()) {
                ...
                for (TISLine line : node.getLines()) {
                    if (line.isCode()) {
                        codeCount++;
                        if (dupeStart + 1 <= codeCount && codeCount <= dupeStart + dupeAmount) {
            ...
            String descr = filename.replace(puzzle.getId() + ".", "")
                                   .replace(".txt", "?DD" + dupeStart + "N" + dupeAmount);
            submit(data, puzzle, descr, null);
        }
    }

    private void genJumpReplacer(@NotNull TISSave save, TISPuzzle puzzle, String filename, TISOperation newJump) {
        for (int jumpPoint = 0; jumpPoint < TISSave.MAX_NODE_INSTR; jumpPoint++) {
            ...
            while (found) {
                            ...
                            if (off != 0 && off != -1 &&
                                ...
                                if (found || countLine <= foundLine) {
                                    ...
                                    if (destLine.getLabel() != null)
                                        data.append(destLine.getLabel());
                                    else
                                        data.append(syntheticLabel);
                                    if (jumpReplaceNotPrepend && line.getComment() != null)
                                        data.append(" #").append(line.getComment());
                                    data.append('\n');
                                    foundLine = countLine;
                                    found = true;
                                    if (jumpReplaceNotPrepend)
                ...
                if (found) {
                    String descr = filename.replace(puzzle.getId() + ".", "")
                                           .replace(".txt", "?" + newJump + jumpPoint + "L" + foundLine);
                    submit(data, puzzle, descr, null);
                }
            }
        }
    ...
    private void genFuseMovMovAcc(TISSave save, TISPuzzle puzzle, String filename) {
        int foundLine = 0;
        boolean found = true;
        while (found) {
            found = false;
    ...
    private void genMovAddSwap(TISSave save, TISPuzzle puzzle, String filename) {
        int foundLine = 0;
        boolean found = true;
        while (found) {
            found = false;
    ...
    private void genJmpExchange(TISSave save, TISPuzzle puzzle, String filename) {
        int foundLine = 0;
        boolean found = true;
        while (found) {
            found = false;
                ...
                for (int i = 0; i < lines.size(); i++) {
                    ...
                    if (line.isCode()) {
                        ...
                        if (found || countLine <= foundLine) {
                            ...
                            TISLine nextLine = nextCodeLine(lines, i);
                            if (nextLine != null && nextLine.is(JMP)) {
            ...
            if (found) {
                String descr = filename.replace(puzzle.getId() + ".", "")
                                       .replace(".txt", "?EX" + foundLine);
                submit(data, puzzle, descr, null);
            }
        }
    }

    private void genJmpExchange2(TISSave save, TISPuzzle puzzle, String filename) {
        int foundLine = 0;
        boolean found = true;
        while (found) {
            found = false;
                ...
                for (int i = 0; i < lines.size(); i++) {
                    ...
                    if (line.isCode()) {
                        ...
                        if (found || countLine <= foundLine) {
                            ...
                            TISLine nextLine = nextCodeLine(lines, i);
                            if (nextLine != null && nextLine.isAny(JEZ, JGZ, JLZ) && line.getOp() != nextLine.getOp() &&
                                line.getJumpTarget().equals(nextLine.getJumpTarget())) {
                                TISOperation missing = TISOperation.values()[JEZ.ordinal() + JGZ.ordinal() + JLZ.ordinal() -
    ...
    private void genFuzzSingle(TISSave save, TISPuzzle puzzle, String filename, int fuzzFactor) {
        if (fuzzFactor == 0)
            return;
        int foundLine = 0;
        boolean found = true;
    ...
    private void genLabelShift(TISSave save, TISPuzzle puzzle, String filename, int labelShift) {
        int foundLine = 0;
        boolean found = true;
        while (found) {
            found = false;
            ...
            if (found) {
                String descr = filename.replace(puzzle.getId() + ".", "")
                                       .replace(".txt", "?SL" + labelShift + "L" + foundLine);
                submit(data, puzzle, descr, null);
            }
        }
    }

    private void genFuzzTries(@NotNull TISSave save, TISPuzzle puzzle, String filename) {
    ...
    private static @NotNull StringBuilder writeLabel(@NotNull StringBuilder data, @NotNull TISLine line) {
        if (line.getLabel() != null)
            data.append(line.getLabel()).append(": ");
        return data;
    }

    private static @Nullable TISLine nextCodeLine(@NotNull List<TISLine> lines, int i) {
        for (int j = i + 1; j < lines.size(); j++) {
            if (lines.get(j).isCode())
                return lines.get(j);
        }
        return null;
    }
...
//        Files.writeString(Path.of("/tmp/res", puzzle.getId() + "-" + descr + ".txt"), data);
        ...
        try {
            submission = TISSubmission.fromData(data.toString(), puzzle, "12345AIII?" + descr, link);
        }
        catch (ValidationException e) {
            return;
        }
        ...
        if (result instanceof SubmitResult.Success<TISRecord, TISCategory> ||
            result instanceof SubmitResult.Updated<TISRecord, TISCategory>)
