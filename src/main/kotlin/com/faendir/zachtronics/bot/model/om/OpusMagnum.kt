...
class OpusMagnum(gitLeaderboard: GitLeaderboard, redditLeaderboard: RedditLeaderboard) : Game<OmCategory, OmScore, OmPuzzle, OmRecord> {
    ...
    override fun parseSubmission(message: Message): Result<Pair<OmPuzzle, OmRecord>> {
        return message.match(regex).flatMap { command ->
            parsePuzzle(command.groups["puzzle"]!!.value).flatMap puzzle@{ puzzle ->
                val scoreString = command.groups["score"]!!.value
                val score = parseScore(puzzle, scoreString) ?: return@puzzle Result.Failure("sorry, I couldn't parse your score ($scoreString).")
                val link = command.groups["link"]?.value ?: message.attachments.firstOrNull()?.url
                ?: return@puzzle Result.Failure("sorry, I could not find a valid link or attachment in your message.")
                Result.Success(puzzle to OmRecord(score, link, message.author.name))
            }
        }
