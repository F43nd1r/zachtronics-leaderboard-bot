...
fun createSubmission(gif: String?, author: String, inputBytes: ByteArray): OmSubmission {
    ...
    JNISolutionVerifier.open(puzzleFile.readBytes(), solutionBytes).use { verifier ->
        ...
        val gifCycles = verifier.getMetricSafe(OmSimMetric.VISUAL_LOOP_START_CYCLE) ?.let { it to verifier.getMetricSafe(OmSimMetric.VISUAL_LOOP_END_CYCLE)!! } ?: (0 to solution.cycles)
        return OmSubmission(
            puzzle,
